document.addEventListener("DOMContentLoaded", () => {
  ...
  function displaySubmittedData() {
    ...
    output.innerHTML = `
      ...
      <p><strong>Name:</strong> ${formData.get("firstName")} ${formData.get("middleName") || ""} "${formData.get("nickname") || ""}" ${formData.get("lastName")}</p>
      <p><strong>Acknowledgment:</strong> ${formData.get("ackStatement")} (${formData.get("ackDate")})</p>
      <p><strong>Mascot:</strong> ${formData.get("mascotAdj")} ${formData.get("mascotAnimal")}</p>
      <p><strong>Divider:</strong> ${formData.get("divider")}</p>
      <img src="${URL.createObjectURL(formData.get("picture"))}" alt="User Image" style="max-width:200px;">
      ...
      <h3>Courses:</h3>
      <ul>
        ${Array.from(coursesContainer.children).map(course => {
          const inputs = course.querySelectorAll("input");
          return `<li>${inputs[0].value} ${inputs[1].value} - ${inputs[2].value} (${inputs[3].value})</li>`;
        }).join("")}
      </ul>
      <blockquote>"${formData.get("quote")}" — ${formData.get("quoteAuthor")}</blockquote>
      ${formData.get("funnyThing") ? `<p><strong>Funny Thing:</strong> ${formData.get("funnyThing")}</p>` : ""}
      ${formData.get("shareThing") ? `<p><strong>Something to Share:</strong> ${formData.get("shareThing")}</p>` : ""}
      <h3>Links:</h3>
      <ul>
        <li><a href="${formData.get("link1")}" target="_blank">${formData.get("link1")}</a></li>
        <li><a href="${formData.get("link2")}" target="_blank">${formData.get("link2")}</a></li>
        <li><a href="${formData.get("link3")}" target="_blank">${formData.get("link3")}</a></li>
        <li><a href="${formData.get("link4")}" target="_blank">${formData.get("link4")}</a></li>
        <li><a href="${formData.get("link5")}" target="_blank">${formData.get("link5")}</a></li>
      </ul>
